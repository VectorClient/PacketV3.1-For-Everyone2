...
Kiero::Status::Enum Kiero::initialize(RenderType::Enum renderType) {
		...
		if (renderType >= RenderType::D3D9 && renderType <= RenderType::D3D12) {
			...
			if (renderType == RenderType::OpenGL) {
				...
				const char *const methodsNames[] = {
					...
					"glClearColor", "glClearDepth", "glClearIndex", "glClearStencil", "glClipPlane", "glColor3b", "glColor3bv", "glColor3d", "glColor3dv", "glColor4f", "glColor4fv", "glColor3i", "glColor3iv",
...
Kiero::Status::Enum Kiero::Bind(uint16_t index, void **original, void *function) {
	assert(original != NULL && function != NULL);
...
void Kiero::unBind(uint16_t index) {
	if (g_renderType != RenderType::None) {
#if KIERO_USE_MINHOOK
